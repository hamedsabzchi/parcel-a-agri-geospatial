...
def bundle_payload():
    paths=[ROOT/p for p in ("stage03/requirements-lock.txt","config/datasets.yml",
        "config/sources/gaez_v5_source_manifest.yml","data/aoi/parcel_a.geojson","docs/stage03_methodology.md")]
    ...
    paths += sorted(p for p in (ROOT/"config/stage03").rglob("*") if p.suffix in {".yml",".json"})
...
def build_notebook():
    ...
    return dict(cells=[dict(cell_type="markdown",metadata={},source=["# Stage 03 — Maps, tables and graphs\n","Run the cell. Supply your Stage 02 ZIP if asked, then approve Google access. Download your results as one ZIP.\n"]),
