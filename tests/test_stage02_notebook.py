...
class TestStage02Notebook(unittest.TestCase):
    def test_notebook_is_simple_and_registry_driven(self) -> None:
        ...
        self.assertIn("#@title Run Stage 02", text)
        ...
        self.assertIn('ee.Authenticate(auth_mode="notebook", force=True)', text)
        self.assertIn('ee.Initialize(project=EARTH_ENGINE_PROJECT)', text)
        self.assertIn("runner.run_all()", text)
        self.assertNotIn("Earth Engine is not connected", text)
